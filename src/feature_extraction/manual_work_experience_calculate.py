...
Usage: src/work_experience_calculate.py --resume_path=<resume_path> --info_path=<info_path> --file_outpath=<file_outpath> --mode=<mode>
...
def main(resume_path, info_path, file_outpath, mode):
    ...
    if mode == 'train':
        ...
        df_final.to_csv(f'{file_outpath}manual_work_exp_test.csv', index=False)
    else:
...
if __name__ == "__main__":
    main(resume_path=opt["--resume_path"], info_path=opt["--info_path"],
         file_outpath=opt["--file_outpath"], mode=opt["--mode"])
