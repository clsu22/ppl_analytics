...
Usage: src/work_experience_calculate.py --resume <resume_path> --info
<info_path> --output <output>
...
--info <info_path> Name of excel containing  hire date and
...
def load_data(resume_path, info_path):
    ...
    """
    resume = pd.read_excel(f"./data/{resume_path}")
    info = pd.read_excel(f"./data/{info_path}")
...
def avg_work_calculater(df, colname_lst, hire="Hire Date"):
    ...
    df_all3 = (df_all2[
        ["employee_code", "work1_company", "work1", "work2_company",
         ...
         "work5", "work6_company", "work6", "work7_company", "avg"]]
        .rename(
...
def main(resume_path, info_path, output):
    ...
    df_final.to_csv(f'./{output}/avg_work_exp_test.csv', index=False)


if __name__ == "__main__":
    main(resume_path=opt["--resume"], info_path=opt["--info"],
         output=opt["--output"])
