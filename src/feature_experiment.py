...
Usage: src/feature_experiment.py --train <train> --test <test> --finalist
<finalist> --output <output>
...
def main(train, test, output, finalist):
    # Load finalist model
    unfitted_finalist = joblib.load(f"./result/{finalist}")
    lr_result_df = pd.read_csv(f"./result/result_lr.csv", index_col=0)
    lr_test_df = pd.read_csv(f"./result/result_lr_test.csv", index_col=0)
    ...
    feat_expr_final_table_lr.to_csv(
        f"./{output}/result_feat_experiment.csv")
