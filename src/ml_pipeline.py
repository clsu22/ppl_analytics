...
Usage: src/ml_pipeline.py --train <train> --test <test> --output
<output>
...
def feature_grouping(output):
    ...
    feat_groups_df.to_csv(f"./{output}/result_features_grouping.csv")
...
def feature_selection(X, y, X_test, y_test, output, numeric_features,
    ...
    weights_lassocv_df.to_csv(f"./{output}/result_lassocv.csv")
    ...
    weights_rfecv_df.to_csv(f"./{output}/result_rfecv.csv")
    ...
    df_trans_sel.to_csv(f"./{output}/result_training_trans_sel.csv",
                        index=False)
    df_test_trans_sel.to_csv(f"./{output}/result_testing_trans_sel.csv",
                             index=False)

    return X_trans_sel_lassocv, X_test_trans_sel_lassocv


def model_selection(X_trans_sel, y, X_test_trans_sel, y_test, output):
    """
    ...
    lr_result_df.to_csv(f"./{output}/result_lr.csv")
    ...
    xgb_randomsearchCV = RandomizedSearchCV(estimator=XGBClassifier(),
    ...
    lr_test_df.to_csv(f"./{output}/result_lr_test.csv")
    print("Finished prediction, saved test result\n")

    cv_test_result = pd.concat([cv_result, test_result], axis=1)
    cv_test_result.to_csv(f"./{output}/result_cv-test-merged.csv")
    ...
    joblib.dump(lr_gridsearchCV.best_estimator_,
                f"./{output}/result_unfitted_finalist.sav")
    finalist = lr_gridsearchCV.best_estimator_.fit(X_trans_sel, y)
    joblib.dump(finalist, f"./{output}/result_fitted_finalist.sav")
...
def finalist_analysis(finalist, X, X_test, X_trans_sel, y, X_test_trans_sel,
    ...
    weights_finalist_df.to_csv(f"./{output}/result_finalist_weights.csv",
                               index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'./{output}/result_confusion_matrix_plot_test.png')
    ...
    plt.tight_layout()
    plt.savefig(f'./{output}/result_roc_curve_test.png')
    ...
    probs_train_df = show_probs(X.employee_code, y,
                                ...
                                dataset="train")
    probs_train_df.to_csv(f'./{output}/result_finalist_probs_train.csv')
    probs_test_df.to_csv(f'./{output}/result_finalist_probs_test.csv')

    # Draw SHAP Summary plot for testing dataset
    plt.figure()
    shap_plot = draw_shap_summary(finalist, X_test_trans_sel, output)
    plt.title('SHAP Summary plot')
    plt.tight_layout()
    plt.savefig(f"./{output}/result_SHAP_summary_plot_test.png")
    print("Finished finalist analysis")


def main(train, test, output):
    # Load data
    train, test = load_data(train, test)

    # Feature grouping
    feat_groups_dict, categorical, numerical = feature_grouping(output)
    ...
    finalist_analysis(finalist, X, X_test, X_trans_sel_lassocv, y,
                      X_test_trans_sel_lassocv, y_test, output)

    print("\n\nAll done")


if __name__ == "__main__":
    main(train=opt["--train"], test=opt["--test"], output=opt["--output"])
